...
export function Footer({ locale = defaultLocale }: FooterProps) {
  ...
  return (
    <footer className="bg-ink text-bone">
      <div className="mx-auto grid max-w-7xl gap-10 px-5 py-14 md:grid-cols-[1.2fr_0.7fr_0.7fr_0.7fr] md:px-8">
        <div>
        ...
        <div>
          <p className="text-sm font-semibold uppercase tracking-[0.2em] text-bronze">{labels.footer.legal}</p>
          <ul className="mt-4 space-y-3">
            {legalRoutes.map((item) => (
              <li key={item.href}>
                <Link className="text-bone/75 transition hover:text-bone" href={localizedInternalHref(item.href, activeLocale)}>
                  {labels.legal[item.key === "ai-transparency" ? "aiTransparency" : item.key]}
                </Link>
              </li>
            ))}
            <li>
              <CookieSettingsButton locale={activeLocale} />
            </li>
          </ul>
        </div>
      </div>
